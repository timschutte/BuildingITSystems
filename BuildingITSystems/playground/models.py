...
def returnAppointmentsUser(start_time: datetime, end_time: datetime, user, full_ap=False, as_dict=False):
    conn = sqlite3.connect('db.sqlite3')
    c = conn.cursor()
    ...
    if as_dict:
        for i in range(len(data)):
            data[i] = dict(zip(i_ap.keys(), data[i]))
            data[i]['start'], data[i]['end'] = data[i].pop('start_time'), data[i].pop('end_time')
            data[i]['start'] = datetime.fromtimestamp(data[i]['start'])
            data[i]['end'] = datetime.fromtimestamp(data[i]['end'])
        return data
    else:
        return data
